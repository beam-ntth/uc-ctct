...
export default function Surveys({ students }) {
                                    ...
                                    <div style={{ width: '100%', height: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'flex-start' }}>
                                        <Link href={`/students/profile?id=${x.id}`}>
                                            <div className='displaySurveyRow' key={`elem_${ind}`}>
                                                ...
                                                <p style={{ width: '15%' }}>{(x.survey && parseInt(x.survey.sentCount) > 0) ? `Sent ${x.survey.sentCount} time${x.survey.sentCount > 1 ? 's' : ''}` : "Not sent"}</p>
                                                <p style={{ width: '15%' }}>{(x.survey && x.survey.lastSent) != "" ? x.survey.lastSent : "N/A"}</p>
                                                <p style={{ width: '15%' }}>{(x.survey && x.survey.responseDate) != "" ? x.survey.responseDate : "No Response"}</p>
                                            </div>
                                        </Link>
