...
export default function ValidateEditStudentDetails(props) {
  ...
  async function createProfile() {
    ...
    for (let i = 0; i < newData.length; i++) {
        ...
        await container.item(newData[i].id, newData[i].id).patch(replaceOperation);
    }
    props.setOpen(false);
    props.setCsv(null);
    props.reload();
          ...
          submittingForm ?
            ...
            (<React.Fragment>
              ...
              <div style={{ width: '100%', height: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'flex-start', fontWeight: 'bold' }}>
                ...
                <p style={{ marginLeft: '1rem', width: '15%' }}>Home Email</p>
                <p style={{ marginLeft: '1rem', width: '15%' }}>New Affiliated Email</p>
              </div>
              <div style={{ width: "100%" }}>
                {
                    ...
                    return (
                        <div key={`record_${ind}`} style={{ width: 'auto', height: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                            <div className='displayStudentValidateRow' key={`elem_${ind}`}>
                                ...
                                <p style={{ marginLeft: '1rem', width: '15%' }}>{x.affiliatedEmail}</p>
                            </div>
                        </div >
                    )
                    })
