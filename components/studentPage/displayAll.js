...
export default function DisplayALL (props) {
  ...
  const [statusFilter, setStatusFilter] = useState(Array(2).fill(""))
  ...
  function searchStudentData(substr) {
    ...
    if (!allEqual(statusFilter)) {
      finalSearch = finalSearch.filter(student => {
        if (student.status) {
          return statusFilter.includes(student.status)
        }
        return false;
      })
    }
  ...
  useEffect(() => {
    searchStudentData('')
  }, [languageFilter, countyFilter, settingFilter, preferencesFilter, statusFilter])
          ...
          <div className={styles.data}>
            ...
            <div className={styles.filterRow}>
              ...
              <div className={styles.regionForm}>
                ...
                <Dropdown disableSearch open={showStatusDropdown} setOpen={setShowStatusDropdown} choices={["Active", "Inactive"]}
                  ...
                  <div style={{ width: '100%', height: 'auto', display: 'flex', alignItems: 'center', justifyContent: 'flex-start' }}>
                    <Link href={`/students/profile?id=${x.id}`}>
                      <div className='displayStudentRow' key={`elem_${ind}`}>
                        ...
                        <p style={{ width: '15%' }}>{x.location_affiliation}</p>
                        <p style={{ width: '15%' }}>{x.status}</p>
                      </div>
                    </Link>
                  </div >
                )
